...
#  RDS security group 
resource "aws_security_group" "allow-restaurants-mysqldb" {
  vpc_id          = var.VPC_ID
  ...
  ingress {
    ...
    to_port         = 3306
    protocol        = "tcp"
    security_groups = [var.RESTAURANTS_SG_ID]
  }
  ...
  tags = {
    Name = "allow-restaurants-mysqldb-${var.SERVICE_NAME}"
  }
}
...
resource "aws_db_instance" "mysqldb" {
  ...
  vpc_security_group_ids  = [aws_security_group.allow-restaurants-mysqldb.id]
